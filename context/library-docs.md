...
### Current auth integration ? 2026-09-13

Use the installed `@insforge/sdk/ssr` helpers from SDK 1.5.2. The previous `@insforge/ssr` examples are superseded.

- Browser: `getInsforgeBrowser()` from `lib/insforge-client.ts` consumes an existing session. Auth mutations belong on the server.
- Server: `await createInsforgeServer()` from `lib/insforge-server.ts`, then `auth.getCurrentUser()`. Use `requireUser()` before protected data access.
- OAuth and sign-out: `createAuthActions({ cookies: await cookies() })` with explicit SDK config. OAuth initiation returns a URL and PKCE verifier; save the verifier in an HTTP-only cookie.
- Callback: `exchangeOAuthCode(code, verifier)` writes session cookies through the response cookie store. SSR does not auto-exchange callbacks in the browser.
- Refresh: `updateSession()` from `@insforge/sdk/ssr/middleware` in `proxy.ts`, plus same-origin POST `/api/auth/refresh`. Forward refreshed request cookies before server rendering.
- Current implementation and deployment configuration: `context/auth-implementation.md`.

The database/storage examples below are historical planning references, not validated SDK 1.5.2 examples. Fetch their current InsForge MCP documentation before implementing those later features.
