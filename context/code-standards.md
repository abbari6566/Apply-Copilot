...
### Auth-specific conventions (2026-09-13)

Auth redirect responses are exceptions to the JSON result envelope. Keep Next.js `redirect()` outside catches because it throws its control-flow signal. Refresh JSON retains the SDK-required top-level `accessToken` field and never exposes the refresh token. Canonical OAuth origin comes from server-only `APP_URL`; set it in deployment configuration.
