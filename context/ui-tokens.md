> Current implementation (2026-09-13): Tailwind CSS 3.4.19 is required and pinned. Colors live in `:root` CSS variables, mapped by `tailwind.config.ts`; PostCSS uses `tailwindcss` and `autoprefixer`. The v4 setup instructions below are retained as original design reference and are superseded by this note. Semantic class names and design values remain unchanged.
