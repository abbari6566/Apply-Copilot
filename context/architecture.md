...
- Public routes: /, /login
- `proxy.ts` refreshes sessions and redirects unauthenticated requests; `requireUser()` validates the user with InsForge before protected rendering/data access
...
Current implementation uses `@insforge/sdk/ssr` from SDK 1.5.2. The earlier separate `@insforge/ssr` package examples are superseded.

- `lib/insforge-server.ts`: `createServerClient({ ...config, cookies: await cookies() })` creates a client per request.
- `lib/insforge-client.ts`: lazy `createBrowserClient({ ...config, refreshUrl: "/api/auth/refresh" })` consumes established sessions.
- `actions/auth.ts`: `createAuthActions()` starts OAuth and signs out on the server.
- `lib/auth-http.ts`: exchanges OAuth callback codes with PKCE and handles same-origin refresh; Route Handlers expose these operations.
- `lib/auth.ts`: validates users through `getCurrentUser()`; use `requireUser()` before protected data access.
- `app/(protected)/`: shared authenticated layout and dashboard route. Future profile and job routes belong here and must still guard data access.

See `context/auth-implementation.md` for the exact flow, configuration, cookie behavior, and validation status.
